...
void oled_task(){
    ...
    while(true){
        if(xSemaphoreTake(xSemaphoreTrigger, portMAX_DELAY) == pdTRUE && xQueueReceive(xQueueDistance, &distancia, pdMS_TO_TICKS(1000)) == pdTRUE){
...
int main() {
    ...
    xQueueTime = xQueueCreate(8, sizeof(int));
